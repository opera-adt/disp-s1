...
JSON_URL = (
    f"https://github.com/opera-adt/burst_db/releases/download/v0.13.0/{DB_FILENAME}"
)
...
class DataStager:
    ...
    def __init__(
        self, runconfig_path: Path, output_dir: Path, aws_profile: str = "saml-pub"
    ):
        """Initialize the OPERA data stager.

        Parameters
        ----------
        runconfig_path : Path
            Path to the runconfig YAML file
        output_dir : Path
            Directory where staged data will be stored
    ...
    def load_burst_db(url: str, frame_id: int) -> pd.DataFrame:
        ...
        logger.info(
            f"  Database generation date: {data['metadata']['generation_time']}"
        )
    ...
    def validate_cslc_coverage(self, cslc_df: pd.DataFrame) -> dict:
        ...
        result = (
            ...
            .agg(
                ...
                db_consist_flag=(
                    "burst_id",
                    lambda x: all(b in db_norm_set for b in x),
                ),
        ...
        self.logger.info(
            f"Runconfig contains {result.shape[0]} dates for frame {self.frame_id}"
        )
        ...
        if result.db_consist_flag.all():
            ...
            for _, row in result.iterrows():
                ...
                if issues:
                    self.logger.warning(
                        f"  Date {row['yyyymmdd']}: {', '.join(issues)}"
                    )
    ...
    def download_config_file(
        self, filename: str, base_url: str, fallback: str | None = None
    ) -> Path:
    ...
    def stage_all_data(self) -> dict:
        ...
        if DISP_S1_INSTALLED:
            config = DownloadConfig(
                input_files=[Path(f) for f in cslc_list],
    ...
    def _stage_dem_basic(self, output_file: Path, bbox: tuple) -> None:
        """Stage DEM using GDAL directly.

        Fallback method when disp_s1 is not installed.
        """
        from osgeo import gdal
        ...
        if src_ds is None:
            raise RuntimeError(
                "Failed to open DEM from S3. Check AWS credentials and network."
            )
    ...
    def _download_config_files(self):
        ...
        try:
            ...
            self.download_config_file(
                ref_file, DISP_CONFIG_URL + "static_ancillary_files/"
            )
        except Exception as e:
    ...
    def create_updated_runconfig(self, staged_files: dict) -> Path:
        ...
        alg_file = next(
            (
                ...
                if "algorithm_parameters" in f.name and "override" not in f.name
            ),
            None,
        )
        ...
        override_file = next((f for f in config_files if "overrides" in f.name), None)
...
def main():
    """Run the OPERA DISP-S1 data staging script."""
